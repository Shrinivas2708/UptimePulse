...
export const addCustomDomain = async (statusPageId: string, domain: string) => {
  const { data } = await apiClient.post('/status-pages/custom-domain', { statusPageId, domain });
  return data;
};

export const verifyCustomDomain = async (domain: string) => {
  const { data } = await apiClient.post(`/status-pages/custom-domain/verify/${domain}`);
  return data;
};
...
export const deleteCustomDomain = async (domain: string): Promise<{ message: string }> => {
  const { data } = await apiClient.delete(`/status-pages/custom-domain/${domain}`);
  return data;
};