...
export const fetchMonitorStats = async (
  ...
  const { data } = await apiClient.get(`/monitors/${monitorId}/stats`, {
    params: { period },
  });
  return data;
};
...
// --- TWILIO ---
export const sendTwilioTestSms = async (
  toNumber: string,
  message: string
): Promise<{ message: string }> => {
  const { data } = await apiClient.post("/integrations/twilio/test-sms", {
    toNumber,
    message,
  });
  return data;
...
export const createNotificationConfig = async (
  ...
  const { data } = await apiClient.post(`/notifications/${monitorId}`, {
    integrationId,
  });
  return data;
};
...
export const fetchStatusPageBySlug = async (
  slug: string
): Promise<IStatusPage> => {
...
export const addCustomDomain = async (statusPageId: string, domain: string) => {
  const { data } = await apiClient.post("/status-pages/custom-domain", {
    statusPageId,
    domain,
  });
  return data;
...
export const verifyCustomDomain = async (domain: string) => {
  const { data } = await apiClient.post(
    `/status-pages/custom-domain/verify/${domain}`
  );
  return data;
};

// ✨ ADD THIS FUNCTION
export const getCustomDomain = async (slug: string) => {
  const { data } = await apiClient.get(`/status-pages/${slug}/custom-domain`);
  return data;
};
export const deleteCustomDomain = async (
  ...
  const { data } = await apiClient.delete(
    `/status-pages/custom-domain/${domain}`
  );
  return data;
};
