...
import {apiClient} from "../lib/axios"; // Import our new axios client
...
function LoginComponent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If successful, store the token in Zustand
      login(token);
    ...
    <div className=" relative text-white min-h-screen flex flex-col max-w-screen-2xl mx-auto z-50">
      ...
      <main className="relative z-20 min-h-screen   flex justify-center items-center">
        <section className="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[400px]  p-5">
          {/* <div className="border p-5 flex flex-col gap-2 items-center"> */}
          ...
          <div className="flex flex-col gap-2">
            <input
              type="email"
              className="flex h-10 w-full bg-[#0c0a09] rounded-md border border-input border-white/10 px-3 py-2 text-sm ring-offset-[#22c55e] file:border-0 file:bg-transparent file:text-xl file:font-medium placeholder:text-[#a1a1aa] focus-visible:outline-none focus-visible:ring-1  focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 outline-none"
